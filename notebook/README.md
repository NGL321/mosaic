...
**The triggers that produced the entry go in that summary too**, for the same reason and
not by omission. The trigger is the one piece of provenance explaining why the entry exists
at all, so it is stated — but a `generated by merge …` line sitting above Noah's first
sentence is the banner wearing a different hat. Provenance belongs with the generated
record it describes, which is inside the fold.
