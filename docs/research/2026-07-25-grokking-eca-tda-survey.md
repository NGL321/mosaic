...
**Ticket:** [#4 — Survey the grokking, ECA, and TDA literatures for a viable first rung](https://github.com/NGL321/mosaic/issues/4)
**Map:** [#1 — Founding charter for Mosaic](https://github.com/NGL321/mosaic/issues/1)
**Date:** 2026-07-25
**Provenance Tier:** machine-produced, unverified. Every claim below is sourced to a primary document (paper, official repository, or publisher record) and linked, but none of the derivations have been checked by Noah unaided. Discharging this is Verification Debt, logged against the Curriculum.

---

## 0. What this survey was for, and what it concluded

The candidate first Protective Belt rung is *how a compressed representation of a concept forms in an inference network*. The candidate instrument is **grokking in elementary cellular automata rule-learning, probed with persistent homology**.

This document asks one question: **can that instrument bear the evidentiary weight?**

The short answer is **no, not as specified** — and the reason is specific and fixable in part. It is not that TDA is unserious, nor that ECA is a bad task. It is that:

1. The single published positive result linking persistent homology to grokking ([Tang et al., 2026](https://arxiv.org/abs/2605.06352)) obtains its signal from the *cyclic group structure of modular arithmetic*, and the same paper reports the signal **degrading to nothing** on a task without that structure (MNIST). ECA rule-learning is Boolean-function learning on a hypercube; it has no analogue of the Z_p loop. The positive precedent does not transfer, and the paper's own negative control predicts it will not.
2. That positive result operates in exactly the regime two 2024 papers prove persistent homology to be unreliable: ~113–197 points in 128 ambient dimensions, i.e. high-dimension/low-sample-size ([Hiraoka et al., 2024/2026](https://arxiv.org/abs/2404.18194)) with Euclidean Vietoris–Rips at d ≫ 30 ([Damrich et al., NeurIPS 2024](https://arxiv.org/abs/2311.03087)).
3. The ECA-grokking literature is **substantially thinner than assumed**, and the Wolfram taxonomy is **provably not a well-characterised taxonomy** in the sense the rung needs ([Culík & Yu, 1988](https://www.semanticscholar.org/paper/Undecidability-of-CA-Classification-Schemes-Cul%C3%ADk-Yu/c754b39540f79fd542f4e067ab7436b720605192)).

Full argument in §4. Sections 1–3 are the evidence.

---

## 1. Grokking and its mechanistic accounts
...
### 1.2 The mechanistic accounts, and where they disagree
...
| Is there a single mechanism? | **No consensus.** Circuit efficiency, lazy→rich, weight-norm/LU, softmax collapse and local-complexity accounts are all live and are not obviously reducible to one another. |
...
## 2. Grokking and rule-learning in elementary cellular automata

### 2.1 The one real reproduction
...
### 2.3 Testing Noah's belief: is the ECA rule taxonomy uniquely suited?
...
## 3. TDA and persistent homology on neural representations
...
### 3.2 The methodological hazards, itemised

#### (a) Sensitivity to choice of filtration
...
#### (b) Sensitivity to choice of metric on representation space
...
#### (c) Behaviour under noise and finite sampling — the decisive hazard
...
#### (d) Statistical significance of barcodes, and what null models exist

They exist, and are almost never used in the neural-representation literature.
...
#### (e) Curse of dimensionality in activation space
...
#### (f) Computational cost at realistic layer widths
...
#### (g) The reproducibility record on this specific application
...
## 4. Implications for the first rung

### 4.1 Can TDA/persistent homology on ECA-grokking representations currently support a falsifiable claim about compressed representation formation?
...
### 4.3 Alternative and complementary instruments for the same rung
...
**Recommended composite for the first rung:** circuit-efficiency predictions (Varma et al.) as the falsification target, with LID and effective rank as the compression measures, an ECA-basis progress measure as the mechanistic layer, and PH/RTD carried as preregistered exploratory secondaries. That combination is defensible on day one and still contains at least two things nobody has done.

### 4.4 Where the genuine open gaps are

Places a careful newcomer could contribute something real:
...
**Cellular automata**
- Culík & Yu (1988). *Undecidability of CA Classification Schemes.* Complex Systems 2, 177–190.
...
- Corneanu, Escalera & Martinez (2020). *Computing the Testing Error without a Testing Set.* CVPR 2020. https://arxiv.org/abs/2005.00450
