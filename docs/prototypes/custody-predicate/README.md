...
## Settled: attendance is a proxy, not the claim

`legitimate` is now `truly_defensible` alone. Requiring attendance made the programme less
defensible than the amanuensis capability the requirement was meant to protect — which
defeats §5's own purpose. `truly_attended` stays as a field, so a false attendance claim is
still visible on screen; it just no longer decides anything.

What that buys, across all 15 cases with the floor declared:

| | SOUND | FOOLED | OVERSTRICT | SILENT |
|---|---|---|---|---|
| A typed | 4 | 1 | **5** | 5 |
| B amanuensis | 5 | **3** | 0 | 7 |
| C endorsed | 1 | 0 | 0 | **14** |
| D aman+warrant | 6 | **0** | 1 | 8 |
