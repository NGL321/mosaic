...
def render_annotation_first(entry: Entry, sel: Selection) -> str:
    ...
    out += [_orphans(sel), ""]
    # The trigger goes *inside* the fold, not above the lede. It is the one piece of
    # provenance explaining why the entry exists, so it is not dropped — but a
    # `<sub>generated by merge …</sub>` line above Noah's first sentence is the GENERATED
    # banner wearing a different hat, and B exists to keep machine front matter off the
    # top of the page. A and C keep it above the fold; they are preserved as driven.
    out += ["<details>\n<summary>Generated record — "
            f"{len(sel.kept)} line(s), {sel.spent} words — "
            + ", ".join(f"{t.kind} {t.label}" for t in entry.triggers) + "</summary>\n"]
    for n in sel.kept:
        out.append(f"- {n.text} {_cites(n)} `{n.badge()}`")
